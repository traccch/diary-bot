...
def _list_text(user: UserSettings, reminders: Sequence[Reminder]) -> str:
    ...
    skip = (
        "🔕 <b>Молчу, если уже сделано.</b> Измерил давление за последние "
        "полтора часа, записал траты, позанимался английским, отметил пробег — "
        "напоминание об этом сегодня не придёт."
        if user.skip_if_measured
        else "🔔 <b>Напоминаю в любом случае</b> — даже если дело уже сделано."
    )
    return (
        f"⏰ <b>{len(reminders)} {word}</b>\n"
        + "\n".join(lines)
        + f"\nЧасовой пояс: {esc(user.tz)} · сменить — /tz\n\n"
        + f"{skip}\n<i>Переключается кнопкой внизу.</i>\n\n"
        + "<i>Нажми на время, чтобы удалить его.</i>"
    )
