...
#: Единицы измерения: число рядом с ними — это литры и километры, а не рубли
#: и не дата. «-833 заправка 13.2л» — восемьсот тридцать три рубля за
#: тринадцать литров, а не тринадцатое февраля.
_UNITS = r"(?:литр\w*|л|мл|кг|км|шт|гр|грамм\w*)"
_NOT_A_MEASURE = r"(?!\s*" + _UNITS + r"(?![\w]))"

_NUMBER = re.compile(
    r"(?<![\w.,])"
    # «23:06-6:30» — это время сна, а не тридцать рублей за шестое число
    r"(?<!\d:)"
    r"(?P<int>\d{1,3}(?:[  ]\d{3})+|\d+)"
    r"(?:[.,](?P<frac>\d{1,2}))?"
    r"\s*(?P<mult>кк|к|k|тыс\.?|тысяч[аи]?|млн|m)?"
    # число не должно обрываться на середине: «13.2л» — это не «13»
    r"(?![\w])(?![.,]\d)(?!:\d)" + _NOT_A_MEASURE,
    re.IGNORECASE,
)

_DATE = re.compile(
    r"(?<![\d.,/])(\d{1,2})[./](\d{1,2})(?:[./](\d{2,4}))?(?![\d.,/])"
    # за датой не может стоять буква: «13.2л» — это литры
    r"(?![^\W\d_])",
    re.IGNORECASE,
)
